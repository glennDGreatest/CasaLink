...
class DataManager {
    ...
    static isGeneratingBills = false; // Lock to prevent concurrent bill generation
    ...
    static async updateMaintenanceRequest(requestId, updates) {
        try {
            ...
            // If request was completed/closed, auto-create a maintenance bill for the tenant
            try {
                const newStatus = (updates.status || '').toString().toLowerCase();
                if (newStatus === 'completed' || newStatus === 'closed') {
                    // Fetch the latest request data
                    const reqDoc = await firebaseDb.collection('maintenance').doc(requestId).get();
                    if (reqDoc.exists) {
                        const request = { id: reqDoc.id, ...reqDoc.data() };

                        // Avoid duplicate bills for same maintenance request
                        const existingBillsSnap = await firebaseDb.collection('bills')
                            .where('maintenanceRequestId', '==', requestId)
                            .get();
                        if (existingBillsSnap && existingBillsSnap.size > 0) {
                            console.log('⚠️ Bill already exists for maintenance request:', requestId);
                        } else {
                            // Determine amount: prefer actualCost, fallback to estimatedCost
                            const amount = parseFloat(request.actualCost || request.estimatedCost || 0) || 0;
                            ...
                            const billData = {
                                ...
                                notes: `Auto-generated from maintenance request ${requestId}`,
                                ...
                                items: [
                                    {
                                        description: `Maintenance charge: ${request.title || 'Service'}`,
                                        amount: amount,
                                        type: 'maintenance'
                                    }
                                ]
                            ...
                            console.log('✅ Auto-generated maintenance bill:', billRef.id, 'for request', requestId);
                        }
                    }
                }
            } catch (billErr) {
                console.error('❌ Error auto-creating maintenance bill:', billErr);
            }
    ...
    static async generateMonthlyBills(options = {}) {
        try {
            // LOCK: Prevent concurrent bill generation calls
            if (this.isGeneratingBills) {
                console.warn('⚠️ Bill generation already in progress. Request rejected to prevent race condition.');
                return {
                    generated: 0,
                    skipped: 0,
                    total: 0,
                    error: 'Bill generation already in progress'
                };
            }
            
            this.isGeneratingBills = true;
            
            const monthOffset = parseInt(options.monthOffset || 0, 10) || 0;
            ...
            const settings = await this.getBillingSettings();
            console.log(`💰 Generating monthly bills for target month: ${targetYear}-${targetMonth + 1}`);
            ...
            for (const lease of leases) {
                try {
                    ...
                    // Log lease info to help diagnose tenant/lease mapping issues
                    console.log(`   - lease: id=${lease.id || 'n/a'}, tenantId=${lease.tenantId || 'n/a'}, tenantName=${lease.tenantName || 'n/a'}, monthlyRent=${lease.monthlyRent}`);

                    // Robust check for existing RENT bills this month.
                    // Fetch tenant bills and inspect any common due fields (`dueDate`, `due`, `due_date`) in JS
                    const tenantBillsSnap = await firebaseDb.collection('bills')
                        .where('tenantId', '==', lease.tenantId)
                        .get();

                    let hasRentBillThisMonth = false;
                    let hasAutoGenRentThisMonth = false;
                    ...
                    tenantBillsSnap.forEach(doc => {
                        ...
                        if (dateField) {
                            try {
                                if (typeof dateField === 'string') billDate = new Date(dateField);
                                else if (dateField && typeof dateField.toDate === 'function') billDate = dateField.toDate();
                                else billDate = new Date(dateField);
                            } catch (e) {
                                billDate = null;
                            }
                        }

                        const billDateStr = billDate && !isNaN(billDate) ? billDate.toISOString() : String(dateField || 'none');
                        const status = (b.status || '').toLowerCase();
                        console.log(`   - bill ${doc.id}: type=${billType}, isAutoGenerated=${!!b.isAutoGenerated}, status=${status || 'unspecified'}, dateField=${billDateStr}`);

                        // FIXED: Block generation ONLY if a RENT bill exists for THIS month
                        // This prevents duplicate rent bills while allowing security deposits and other bill types
                        // Check ONLY for the target month - do NOT check next month
                        if (billType === 'rent' && billDate && billDate.getFullYear() === targetYear) {
                            const billMonth = billDate.getMonth();
                            // Check ONLY if bill is in target month - this allows recreation after deletion
                            if (billMonth === targetMonth) {
                                hasRentBillThisMonth = true;
                                if (b.isAutoGenerated) hasAutoGenRentThisMonth = true;
                                console.log(`     -> Found existing ${status} RENT bill for current month; generation will be blocked`);
                            }
                        }
                    });

                    console.log(`   -> hasRentBillThisMonth=${hasRentBillThisMonth}, hasAutoGenRentThisMonth=${hasAutoGenRentThisMonth}`);

                    if (!hasRentBillThisMonth && !hasAutoGenRentThisMonth) {
                        ...
                        console.log(`   ℹ️ Bill details: lease.paymentDueDay=${lease.paymentDueDay}, fallback=${settings?.defaultPaymentDay}, final paymentDay=${paymentDay}, dueDate=${dueDate.toISOString()}`);
                        ...
                        const finalCheckSnap = await firebaseDb.collection('bills')
                            .where('tenantId', '==', lease.tenantId)
                            .where('type', '==', 'rent')
                            .where('dueDate', '>=', new Date(targetYear, targetMonth, 1).toISOString())
                            .where('dueDate', '<=', new Date(targetYear, targetMonth + 1, 0).toISOString())
                            .limit(1)
                            .get();
                        ...
                        skippedCount++;
                        if (hasAutoGenRentThisMonth) {
                            console.log(`⏭️ Auto-generated RENT bill already exists for ${lease.tenantName} for target month (race condition protection)`);
                        } else if (hasRentBillThisMonth) {
                            console.log(`⏭️ RENT bill already exists for ${lease.tenantName} for current/next month - generation blocked to prevent duplicates`);
                        } else {
                            console.log(`⏭️ Skipped generating bill for ${lease.tenantName}`);
                        }
                    }
                } catch (error) {
            ...
            // UNLOCK: Release the lock when done
    ...
    // Normalize bill amount fields across different record shapes
    static normalizeBill(d = {}) {
        const normalizedTotal = d.totalAmount ?? d.amount ?? 0;
        const normalizedAmount = d.amount ?? d.totalAmount ?? 0;
    ...
    static async createBill(billData) {
        ...
        // Create activity for bill generation (skip for auto-generated bills)
        if (!billData.isAutoGenerated) {
            try {
    ...
    static async logActivity(landlordId, activity) {
        ...
        const payload = {
            ...
            isSeen: activity.isSeen === 'seen' ? 'seen' : 'unseen'
        };

        return this.addActivity(payload);
    }

    static async addActivity(activityData) {
        if (!activityData) throw new Error('activityData is required');

        const payload = {
            ...activityData,
            createdAt: activityData.createdAt || new Date().toISOString(),
            timestamp: activityData.timestamp || activityData.createdAt || new Date().toISOString(),
            isSeen: activityData.isSeen === 'seen' ? 'seen' : 'unseen'
        };
